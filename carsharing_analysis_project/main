...
# Not all the fields from JSON file required. 'request_nr' and 'weekend' left out for memory savings.
...
"""
# Commented because index has been successfully created and ingested
...
for hit in scan(client=es, index=index_name, query=all_query):
    ...
    if origin_location != destination_location and origin_datetime != destination_datetime:
        ...
        if 5 <= speed_kmh <= 130:  # For urban areas, speed typically ranges from 5km/h to 130km/h
...
# PLOTTING TIME SERIES OF DAILY DEMAND
...
# Average speed in km/h and revenue rate in € per started minute
average_speed_kmh = 50
revenue_rate = 0.19

# Calculate profit (based on formula distance / speed)
...
# Define parameters
number_cars = len(cars_location_df)
number_requests = len(requests_data_df)

# Pre-calculate distances and compatibility
distances = np.zeros((number_cars, number_requests))
compatibility = np.zeros((number_cars, number_requests), dtype=bool)

for c, car in enumerate(cars_location_df):
    car_location = tuple(car['start_location'])
    for j, request in enumerate(requests_data_df):
        request_origin = tuple(request['origin_location'])
        distance = calculate_distance(car_location, request_origin).kilometers
        distances[c, j] = distance
        compatibility[c, j] = distance <= 0.4  # 400 meters walking distance

# Initialize optimization model
model = Model("Car sharing Optimization")

# Decision variables
x = model.addVars(number_cars, number_requests, vtype=GRB.BINARY, name="x")

# Objective function
model.setObjective(sum(x[c, j] * requests_data_df[j]['profit']
                       for c in range(number_cars) for j in range(number_requests)), GRB.MAXIMIZE)

# Constraints
for j in range(number_requests):
    model.addConstr(sum(x[c, j] for c in range(number_cars)) <= 1)

for c in range(number_cars):
    model.addConstr(sum(x[c, j] for j in range(number_requests)) <= 1)

# Apply compatibility based on pre-calculated distances
for c in range(number_cars):
    for j in range(number_requests):
        if not compatibility[c, j]:
            model.addConstr(x[c, j] == 0)
...
for c in range(number_cars):
    for j in range(number_requests):
        if x[c, j].X > 0.5:
            matched_assignments[c] = j
            print(f"Request {j} assigned to car {c}")

print(f"The number of matched requests is {len(matched_assignments)}")

# Initialize lists to store latitude and longitude values for matched requests
matched_lats = []
matched_lons = []

# Iterate over matched assignments to collect latitude and longitude values for matched requests
for car_index, request_index in matched_assignments.items():
...
for c, j in matched_assignments.items():
...
# Extracting unmatched requests
unmatched_requests_indices = set(range(number_requests)) - set(matched_assignments.values())
...
if unmatched_lats and unmatched_lons:
    unmatched_map = smopy.Map((min(unmatched_lats), min(unmatched_lons), max(unmatched_lats), max(unmatched_lons)),
                              z=12)
...
# Define initial variables where solutions are to be stored
walking_distances = [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55,
                     0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1.0]
profits = []
compatible_customers_counts = []
...
# Assuming the rest of your setup is as previously discussed
...
upper_bound = profits + profit_variability
lower_bound = profits - profit_variability
...
plt.xticks(walking_distances, rotation=45, fontsize=12)  # Adjust for better readability
